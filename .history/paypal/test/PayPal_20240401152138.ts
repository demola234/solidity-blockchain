...
describe("Paypal", function () {
  ...
  describe("Paypal Contract", function () {
    ...
    it('Should pay request correctly', async function () {
      ...
      await payPal.connect(otherAccount).createRequest(amount, message, owner.address);
      await expect(() => payPal.connect(otherAccount).payRequest(0, { value: amount })).to.changeEtherBalance(owner, amount);
    });
